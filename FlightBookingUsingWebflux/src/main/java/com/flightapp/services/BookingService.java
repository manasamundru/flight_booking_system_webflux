...
public class BookingService {
	...
	private Mono<String> validateAndCreateBooking(Flights flight, BookingRequest req) {
		Set<String> requestedSeats = req.getPassengers().stream().map(p -> p.getSeatNumber())
				.collect(Collectors.toSet());
		if (requestedSeats.size() != req.getPassengers().size()) {
			return Mono.error(new IllegalArgumentException("no of seats doesnt match the no of passengers"));
		}
		return bookingRepo.findByFlightId(flight.getId()).collectList().flatMap(existingBookings -> {

			Set<String> occupiedSeats = existingBookings.stream().flatMap(b -> b.getPassengers().stream())
					.map(Passenger::getSeatNumber).collect(Collectors.toSet());
			...
			booking.setPassengers(req.getPassengers().stream()
					.map(p -> new Passenger(p.getName(), p.getGender(), p.getAge(), p.getSeatNumber()))
					.collect(Collectors.toList()));
			booking.setPricePaid(seatsToBook * flight.getPriceOneWay());
			return flightService.decrementSeats(flight.getId(), seatsToBook)
					.flatMap(updated -> bookingRepo.save(booking)).map(saved -> saved.getPnr());
		});
	}
